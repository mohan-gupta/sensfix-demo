...
async def categorize_and_respond(user_complaint: str):
    # Validation
    validation_result = validation_chain.run(user_complaint)
    ...
    first_category = await asyncio.to_thread(first_category_chain.run, user_complaint)
    ...
    second_category = await asyncio.to_thread(second_category_chain.run, user_complain=first_category)
    ...
    response = await asyncio.to_thread(response_chain.run, user_complaint=user_complaint)
